...
import { useCreateBox } from '@/hooks/useInward'
...
export function CreateBoxPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      boxHistory.add(created.box_id, created.customer_id)
      setBox(created)
    } catch (err) {
      ...
      <div className="max-w-md mx-auto">
        <Card>
          <div className="text-center mb-6">
            ...
            <p className="text-gray-500 text-sm mt-1">Ready to scan items into this box.</p>
          </div>
          ...
          <div className="bg-gray-50 rounded-lg p-4 mb-6 space-y-2">
            ...
            <div className="flex justify-between text-sm">
              ...
              <Badge variant="yellow">Scanning</Badge>
            </div>
          </div>

          <div className="space-y-3">
            <Button className="w-full" onClick={() => navigate(`/scan?box=${box.box_id}`)}>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-2 mb-1">
            ...
            <CardTitle>Create Scanning Box</CardTitle>
          </div>
          <p className="text-sm text-gray-500">
            Open a new box for a customer. Only one active scanning box is allowed per packer at a
            time.
          </p>
        </CardHeader>

        {error && (
          <Alert variant={error.includes('Mark') ? 'warning' : 'error'} className="mb-5">
            {error}
          </Alert>
        )}
